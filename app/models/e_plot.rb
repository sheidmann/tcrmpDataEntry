  ...
  def self.as_csv
    ...
    CSV.generate(headers: true) do |csv|
      ...
      all.each do |p|
        esurv = p.e_survey
        diver = esurv.user
        csv << [esurv.fid, esurv.proofed, esurv.date_completed, diver.name, esurv.buddy, esurv.team, esurv.role, esurv.habitat, p.hardbottom, p.coral_cover, p.max_relief_cm, p.min_depth, p.max_depth]
      end
    end
  end
end
